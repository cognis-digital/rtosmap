"""Native cognis-connect emit for rtosmap — forward findings to any platform.

Maps rtosmap's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    rtosmap ... --format json | rtosmap-emit --to stix
    rtosmap-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('task', '') + ' Stack Overflow'
        out['severity'] = 'high' if rec.get('used_pct', 0) > 80 else 'medium'
        out['type'] = 'Stack Overflow'
        out['description'] = f"Task '{rec.get('task', '')}' uses {rec.get('used_pct', 0):.1f}% of its stack"
        out['tags'] = ['stack', 'overflow']
